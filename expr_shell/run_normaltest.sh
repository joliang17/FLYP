...
#SBATCH --job-name=flyp_loss_all
#SBATCH --output=flyp_loss_all.out.%j
#SBATCH --error=flyp_loss_all.out.%j
...
python src/main.py --train-dataset=IWildCamIDVal --epochs=2 --lr=1e-5 --wd=0.2 --batch-size=64 --model=ViT-B/16 --eval-datasets=OxfordPetTest --template=iwildcam_template  --save=./checkpoints/ --data-location="./datasets/data/" --ft_data="${SAVED_FOLDER}train.csv" --ft_data_test="${SAVED_FOLDER}curriculum.csv" --csv-img-key filepath --csv-caption-key title --exp_name=debug --workers=4 --debug --test --cont_finetune
