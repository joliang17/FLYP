...
#SBATCH --job-name=clip_lyj
#SBATCH --output=clip_lyj.out.%j
#SBATCH --error=clip_lyj.out.%j
...
SAVED_FOLDER="../data/metadata/clip/"
...
python src/main.py --train-dataset=IWildCamIDVal --epochs=20 --lr=1e-5 --wd=0.2 --batch-size=256 --model=ViT-B/16 --eval-datasets=IWildCamIDVal --template=iwildcam_template  --save=./checkpoints/ --data-location="../data/iwildcam" --ft_data="${SAVED_FOLDER}train.csv" --ft_data_test="${SAVED_FOLDER}test.csv" --csv-img-key filepath --csv-caption-key title --exp_name=iwildcam/flyp_loss --curriculum --self_data