...
TRAIN_FOLDER="../data/imagenet/train_new"
...
# python src/main.py --train-dataset=ImageNet --epochs=10 --lr=1e-5 --wd=0.1 --batch-size=512 --model=ViT-B/16 --eval-datasets=ImageNet,ImageNetV2,ImageNetR,ImageNetA,ImageNetSketch,ObjectNet  --template=openai_imagenet_template_reduced  --save=./checkpoints/ --data-location="../data/imagenet" --ft_data="${SAVED_FOLDER}train.csv" --ft_data_test="${SAVED_FOLDER}test.csv" --csv-img-key filepath --csv-caption-key title --exp_name="flyp_loss_imgnet_base"  --slurm_job_id=$SLURM_JOB_ID 
