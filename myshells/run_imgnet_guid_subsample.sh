...
SAVED_FOLDER="../data/imagenet/metadata/img_guid_no10/"
...
python src/main.py --train-dataset=ImageNet --epochs=20 --lr=1e-5 --wd=0.1 --batch-size=300 --model=ViT-B/16 --eval-datasets=ImageNetLT  --template=openai_imagenet_template_reduced  --save=/fs/nexus-projects/wilddiffusion/gene_diffcls/FYLP/checkpoints/ --data-location="${ORI_DATA}" --ft_data="${SAVED_FOLDER}train.csv" --ft_data_curri="${SAVED_FOLDER}curriculum.csv"  --ft_data_test="${SAVED_FOLDER}test.csv" --csv-img-key filepath --csv-caption-key title --exp_name="flyp_loss_imgnet_guid_no10_sub" --subsample --curriculum --curriculum_epoch=15 --progress_guid --uniform_set --merge_ori --progress_metric=Prob --slurm_job_id=$SLURM_JOB_ID --wandb_tag="image_net"
