...
#SBATCH --job-name=v6042
#SBATCH --output=v6042.out.%j
#SBATCH --error=v6042.out.%j
...
SAVED_FOLDER="../data/metadata/clip_progress_difficult_2022_42/"
...
python src/main.py --train-dataset=IWildCamIDVal --epochs=20 --lr=1e-5 --wd=0.2 --batch-size=200 --model=ViT-B/16 --eval-datasets=IWildCamIDVal,IWildCamID,IWildCamOOD --template=iwildcam_template  --save=./checkpoints/ --data-location="../data/iwildcam/" --ft_data="${SAVED_FOLDER}train.csv" --ft_data_test="${SAVED_FOLDER}train.csv" --csv-img-key filepath --csv-caption-key title --exp_name="flyp_loss_v6042" --curriculum --curriculum_epoch=10 --progress_eval --progress_train --explore --scheduler=default --workers=4 --progress_metric="Acc" --progress --slurm_job_id=$SLURM_JOB_ID
