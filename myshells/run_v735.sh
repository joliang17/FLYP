...
#SBATCH --job-name=v735_3
#SBATCH --output=v735_3.out.%j
#SBATCH --error=v735_3.out.%j
...
SAVED_FOLDER="../data/metadata/clip_progress_difficult_2022_5_onlyguid/"
...
# moving average with all guid, uniform dataset
python src/main.py --train-dataset=IWildCamIDVal --epochs=20 --lr=1e-5 --wd=0.2 --batch-size=300 --model=ViT-B/16 --eval-datasets=IWildCamIDVal,IWildCamID,IWildCamOOD --template=iwildcam_template  --save=./checkpoints/ --data-location="../data/iwildcam/" --ft_data="${SAVED_FOLDER}train.csv" --ft_data_test="${SAVED_FOLDER}curriculum.csv" --csv-img-key filepath --csv-caption-key title --exp_name="flyp_loss_v735_3" --curriculum --curriculum_epoch=15 --progress_guid --uniform_set --progress_metric=Prob --ma_progress --scheduler=default --workers=4 --slurm_job_id=$SLURM_JOB_ID
