...
#SBATCH --job-name=flyp_loss_curri_crestart
#SBATCH --output=flyp_loss_curri_crestart.out.%j
#SBATCH --error=flyp_loss_curri_crestart.out.%j
...
python src/main.py --train-dataset=IWildCamIDVal --epochs=1 --lr=1e-5 --wd=0.2 --batch-size=256 --model=ViT-B/16 --eval-datasets=IWildCamIDVal,IWildCamID,IWildCamOOD --template=iwildcam_template  --save=./checkpoints/ --data-location="./datasets/data/" --ft_data="${SAVED_FOLDER}train.csv" --ft_data_test="${SAVED_FOLDER}curriculum.csv" --csv-img-key filepath --csv-caption-key title --exp_name=flyp_loss_curri_crestart --progress_eval --curriculum --curriculum_epoch=5 --scheduler=crestart --workers=4
