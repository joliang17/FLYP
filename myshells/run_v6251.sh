...
#SBATCH --job-name=flyp_loss_v6152
#SBATCH --output=flyp_loss_v6152.out.%j
#SBATCH --error=flyp_loss_v6152.out.%j
...
# uniform distributed dataset + no randomness
python src/main.py --train-dataset=IWildCamIDVal --epochs=20 --lr=1e-5 --wd=0.2 --batch-size=300 --model=ViT-B/16 --eval-datasets=IWildCamIDVal,IWildCamID,IWildCamOOD --template=iwildcam_template  --save=./checkpoints/ --data-location="../data/iwildcam/" --ft_data="${SAVED_FOLDER}train.csv" --ft_data_test="${SAVED_FOLDER}train.csv" --csv-img-key filepath --csv-caption-key title --exp_name="flyp_loss_v6251" --curriculum --curriculum_epoch=10 --progress_eval --progress_train --scheduler=default --workers=4 --progress_metric="Acc" --progress --slurm_job_id=$SLURM_JOB_ID --cont_finetune --uniform_set
