...
def main(args):
    ...
    if args.mode in ('train', 'curriculum'):
        ...
        label_to_name = pd.read_csv(f'{args.data_folder}/datasets/iwildcam_metadata/labels.csv')

    else:
        # test data
        label_to_name = pd.read_csv(f'{args.data_folder}/datasets/iwildcam_metadata/labels_new.csv')
    ...
    if args.mode == 'train':
        if args.curriculum:
            clip_path = f'{args.data_folder}/data/metadata/clip_score_train.pkl'
            threshold = 0.25
            Dict_filt = filter_img(clip_path, threshold)

            if args.topk_hard:
                topk_path = f'{args.data_folder}/data/metadata/clip_progress_train_img/generate.csv'
        ...
        if not args.total_train:
            ...
            df_train_ori = pd.read_csv(f'{args.data_folder}/datasets/csv/iwildcam_v2.0/train.csv', sep='\t')
        ...
        raise ValueError(f"No test supported!")
        """
        # # test eval
